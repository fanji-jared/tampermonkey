...
// @version      1.5.1
// @description  Abaka标注平台多功能工具箱 - 支持定义审查频率
// @author       You
...
(function() {
                    ...
                    <div class="abaka-tool-settings-content">
                        <div class="abaka-settings-controls">
                            <div class="abaka-controls-wrapper">
                                ...
                                <div class="abaka-frequency-setting">
                                    <label for="abaka-frequencyInput">检测频率 (毫秒):</label>
                                    <input type="number" id="abaka-frequencyInput" value="5000" min="1000" max="60000" step="1000">
                                </div>
                            </div>
                        </div>
    ...
    function setupToolEvents() {
        ...
        // 检测频率设置
        const frequencyInput = document.getElementById('abaka-frequencyInput');
        if (frequencyInput) {
            // 加载保存的频率设置
            const savedFrequency = GM_getValue('abaka-autoClick-frequency', 5000);
            frequencyInput.value = savedFrequency;
            toolManager.autoClick.detectionFrequency = savedFrequency;

            // 监听频率变化
            frequencyInput.addEventListener('input', function(e) {
                const newFrequency = parseInt(e.target.value);
                if (newFrequency >= 1000 && newFrequency <= 60000) {
                    toolManager.autoClick.detectionFrequency = newFrequency;
                    GM_setValue('abaka-autoClick-frequency', newFrequency);
                    addAutoClickerLog(`检测频率已更新为 ${newFrequency} 毫秒`, 'info');
                }
            });
            ...
            frequencyInput.addEventListener('change', function(e) {
                ...
                if (newFrequency < 1000) {
                    e.target.value = 1000;
                    toolManager.autoClick.detectionFrequency = 1000;
                    GM_setValue('abaka-autoClick-frequency', 1000);
                    addAutoClickerLog(`检测频率已调整为最小值 1000 毫秒`, 'warning');
                } else if (newFrequency > 60000) {
                    e.target.value = 60000;
                    toolManager.autoClick.detectionFrequency = 60000;
                    GM_setValue('abaka-autoClick-frequency', 60000);
                    addAutoClickerLog(`检测频率已调整为最大值 60000 毫秒`, 'warning');
                }
            });
        }
    }
    ...
    function exportSettings() {
        const settings = {
            ...
            autoClickFrequency: GM_getValue('abaka-autoClick-frequency', 5000)
        };
    ...
    function importSettings(file) {
        ...
        reader.onload = function(e) {
            try {
                ...
                // 应用检测频率设置
                if (settings.autoClickFrequency && settings.autoClickFrequency >= 1000 && settings.autoClickFrequency <= 60000) {
                    GM_setValue('abaka-autoClick-frequency', settings.autoClickFrequency);
                    const frequencyInput = document.getElementById('abaka-frequencyInput');
                    if (frequencyInput) {
                        frequencyInput.value = settings.autoClickFrequency;
                    }
                    if (toolManager.autoClick) {
                        toolManager.autoClick.detectionFrequency = settings.autoClickFrequency;
                    }
                }
    ...
    function resetSettings() {
        ...
        // 重置检测频率为默认值
        GM_setValue('abaka-autoClick-frequency', 5000);
        const frequencyInput = document.getElementById('abaka-frequencyInput');
        if (frequencyInput) {
            frequencyInput.value = 5000;
        }
        if (toolManager.autoClick) {
            toolManager.autoClick.detectionFrequency = 5000;
        }
    ...
    const toolManager = {
        ...
        autoClick: {
            ...
            init: function() {
                ...
                this.detectionFrequency = 5000; // 默认5000毫秒
            ...
            startListening: function() {
                ...
                this.checkTimer = setInterval(() => {
                    ...
                    if (now - this.lastConsoleTime > this.detectionFrequency) {
                        this.isProcessing = true;
    ...
    GM_addStyle(`
        ...
        .abaka-frequency-setting label {
            flex-shrink: 0;
        }

        .abaka-frequency-setting input[type="number"] {
            width: 80px;
            padding: 4px 6px;
            border: 1px solid #4a5568;
            border-radius: 4px;
        ...
        .abaka-frequency-setting input[type="number"]:focus {
            outline: none;
            border-color: #48bb78;
    ...
    // 初始化工具箱
    createToolboxUI();
    addLog('Abaka标注平台工具箱 v1.5.1 已加载');
